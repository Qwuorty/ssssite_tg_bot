...
async def drink(
    ...
    photo_name = kb.sql.execute(
        f"SELECT photo FROM menu WHERE id='{callback_data.drink_id}'").fetchone()[0]
    desk = kb.sql.execute(
        f"SELECT desk FROM menu WHERE id='{callback_data.drink_id}'").fetchone()[0]
    price = str(kb.sql.execute(
        f"SELECT cost FROM menu WHERE id='{callback_data.drink_id}'").fetchone()[0])
    desk += '\n'+f'<b>{price}</b> ₽'
    try:
        media = types.InputMediaPhoto(media=FSInputFile(f'media/drinks/{photo_name}'),
                                      caption=desk)
        await call.message.edit_media(media=media, reply_markup=kb.drink_kb(callback_data.drink_id))
    except Exception:
        await call.message.answer_photo(photo=FSInputFile(f'media/drinks/{photo_name}'),
                                        caption=desk,
                                        reply_markup=kb.drink_kb(callback_data.drink_id))
    await call.answer()

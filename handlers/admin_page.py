...
from texts import HELLO_TEXT, points
...
async def comeback(
    ...
    if callback_data.oper == 'back':
        ...
        point_id = 1
        await call.message.edit_text(text=f'Стоп лист для точки по адресу {points[point_id]}', reply_markup=kb.get_stops(point_id))
